...
const templatingWithMustache = () => {
  // Solution code here...Write a function named templatingWithMustache that uses mustache to create the markup templates for each of the characters. Use the snippet as your guide for creating your templates. Return an array of template strings. Note: this function does not need to actually append the markup to the DOM.
  let templateStrings = [];
  characters.forEach(throne => {
    templateStrings.push(Mustache.render($('#template').html(), throne));
  })
...
const hasChildrenValues = (arr, character) => {
  ...
  let padres = false;
  arr.forEach(mapa => {
    if (mapa.name === character) {
      padres = mapa.children ? true : false;
    }
  })
  return padres;
}
