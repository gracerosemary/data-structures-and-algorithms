...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => {
    let greeting = 'Trick or treat!';
    res.status(200).send(greeting);
  });

  app.get('/aboutme', (req, res) => {
    let bio = 'My name is Grace and I like treats.';
    res.status(200).send(bio);
  });

  app.get('/favoritefoods', (req, res) => {
    let foodArray = ['pho', 'shin ramen', 'fried chicken', 'papaya salad', 'fries', 'burritos'];
    res.status(200).send(foodArray);
  });

  app.get('*', (req, res) => {
    res.status(404).send('Boo! No treats here.');
  });
